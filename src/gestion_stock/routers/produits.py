...
from gestion_stock.models import Commercant, Produit
...
def create_produit(commercant_id: int, produit: Produit, session: Session = Depends(get_session)):
...
def list_produits(commercant_id: int, session: Session = Depends(get_session), skip: int = 0, limit: int = 100):
    _verifier_commercant(session, commercant_id)
    return session.exec(
        select(Produit)
        .where(Produit.commercant_id == commercant_id, Produit.actif)
        .offset(skip)
        .limit(limit)
    ).all()
...
def get_produit(commercant_id: int, produit_id: int, session: Session = Depends(get_session)):
    _verifier_commercant(session, commercant_id)
...
def update_produit(commercant_id: int, produit_id: int, updates: Produit, session: Session = Depends(get_session)):
    _verifier_commercant(session, commercant_id)
    produit = session.exec(
        select(Produit).where(
            Produit.id == produit_id,
...
def delete_produit(commercant_id: int, produit_id: int, session: Session = Depends(get_session)):
    _verifier_commercant(session, commercant_id)
    produit = session.exec(
        select(Produit).where(
            Produit.id == produit_id,
