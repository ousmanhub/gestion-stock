...
def test_list_entrepots(client, commercant, entrepot):
    response = client.get(f"/commercants/{commercant['id']}/entrepots/")
...
def test_get_entrepot(client, commercant, entrepot):
    response = client.get(f"/commercants/{commercant['id']}/entrepots/{entrepot['id']}")
    assert response.status_code == 200
    assert response.json()["id"] == entrepot["id"]


def test_update_entrepot(client, commercant, entrepot):
    response = client.patch(f"/commercants/{commercant['id']}/entrepots/{entrepot['id']}", json={
...
def test_delete_entrepot(client, commercant, entrepot):
    response = client.delete(f"/commercants/{commercant['id']}/entrepots/{entrepot['id']}")
